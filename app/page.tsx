...
const cardSound = typeof Audio !== "undefined"
  ? new Audio("/sounds/card.wav")
  : null;
...
const loseSound = typeof Audio !== "undefined"
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Sign in to access your account and balance
        </p>
...
function Game() {
 ...
 function applyBalanceChange(delta: number) {
  setBalance((b) => {
    const newBal = clampMoney(b + delta);
  ...
  fetch("/api/balance")
    ...
    .then(data => {
      if (typeof data.balance === "number") {
        ...
        setSessionStartBalance(data.balance); // 👈 capture once
      }
    });
}, []);
...
useEffect(() => {
 ...
 function placeBoardChip(key: BoardBetKey) {
  ...
  setBoardBets((prev) => ({
    ...prev,
    [key]: clampMoney((prev[key] ?? 0) + selectedChip),
  }));

  applyBalanceChange(-selectedChip);
}
 ...
 function clearBoardRefund() {
  ...
  const refund = boardTotal();

  setBalance((b) => {
    const newBal = clampMoney(b + refund);
  ...
  const refunded = boardTotal();

  setBalance((b) => {
    const newBal = clampMoney(b + refunded);
 ...
 applyBalanceChange(totalPayout);
  ...
  sessionStartBalance === null
    ? 0
    : clampMoney(balance - sessionStartBalance);
...
>
  {lastProfit >= 0 ? "+" : "-"}£{Math.abs(lastProfit).toFixed(2)}
              ...
              <div style={{ color: "#64748b" }}>Place chips on the board, then press <b>Deal</b>.
